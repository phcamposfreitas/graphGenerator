...
@Service
@Slf4j
public abstract class TrafegoPacotesCalculadorEstatistico {

	@Getter
	private String nomeInformacaoProcessamento = "Tráfego em Pacotes";
	...
	private void calcular(TotalizadorSessaoDto totalizador, List<SessaoEntity> sessoes, List<String> categorias) {
		for(SessaoEntity sessao : sessoes) {
			...
			if(trafego == null){
				TotalizadorTrafegoDto trafegoNovo = new TotalizadorTrafegoDto(sessao.getPacotesRecebidos(), sessao.getPacotesEnviados());
				mapIntervalos.put(intervalo, trafegoNovo);
			}else{
				trafego.add(sessao.getPacotesRecebidos(), sessao.getPacotesEnviados());
			}
		}
		...
		for(String categoria : categorias){
			OptionalDouble mediaIn = mapIntervalos.entrySet().stream()
												   ...
												   .mapToDouble(entry -> entry.getValue().getMediaIn())
												   .average();
			
			OptionalDouble mediaOut = mapIntervalos.entrySet().stream()
															   .filter(entry -> isSameIntervalo(entry.getKey(), categoria))
															   .mapToDouble(entry -> entry.getValue().getMediaOut())
															   .average();
		...
		for(String categoria : categorias){
			...
			Double somatorioIn = mapIntervalos.entrySet()
							   .stream()
	   					       .filter(entry -> isSameIntervalo(entry.getKey(), categoria))
	   					       .mapToDouble(entry -> (entry.getValue().getMediaIn() - media) * (entry.getValue().getMediaIn() - media))
	   					       .sum();
			
			Double somatorioOut = mapIntervalos.entrySet()
					   .stream()
				       .filter(entry -> isSameIntervalo(entry.getKey(), categoria))
				       .mapToDouble(entry -> (entry.getValue().getMediaOut() - mediaOut) * (entry.getValue().getMediaOut() - mediaOut))
				       .sum();
